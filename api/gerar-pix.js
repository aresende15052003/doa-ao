...
const ALLOWED_VALUES = new Set([10, 30, 50, 75, 100, 150, 200, 300, 500, 1000]);
...
module.exports = async function handler(req, res) {
  try {
    ...
    let raw = '';
    try {
      raw = await gatewayResponse.text();
    } catch (err) {
      return json(res, 502, {
        ok: false,
        message: 'Não foi possível ler a resposta da AmploPay.'
      });
    }
    ...
    if (!raw || !raw.trim()) {
      return json(res, 502, {
        ok: false,
        message: `A AmploPay respondeu sem conteúdo (HTTP ${gatewayResponse.status}).`
      });
    }
    ...
    try {
      ...
      return json(res, 502, {
        ok: false,
        message: `A AmploPay retornou uma resposta inválida (HTTP ${gatewayResponse.status}).`,
        details: raw.replace(/\s+/g, ' ').slice(0, 180)
      });
    }
    ...
    if (!gatewayResponse.ok) {
      return json(
        res,
        gatewayResponse.status >= 400 && gatewayResponse.status < 500 ? 400 : 502,
        {
          ok: false,
          message:
            data.message ||
    ...
    return json(res, 500, {
      ...
      details: err && err.message ? err.message : 'Erro inesperado'
    });
  }
};
